...
# 4. Logs sem erros
ERRORS=$(ssh marlon@$IP 'journalctl -u chirpstack -n 30 --no-pager 2>/dev/null' | grep -ci "error\|panic" || echo 0)
if [ "$ERRORS" -gt 0 ]; then
  echo "ALERTA: $ERRORS erros nos logs do chirpstack"
fi

# 5. Codecs (local)
if ls templates/codecs/tests/test-*.js >/dev/null 2>&1; then
  for f in templates/codecs/tests/test-*.js; do
